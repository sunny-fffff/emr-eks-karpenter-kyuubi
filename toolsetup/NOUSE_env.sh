...
export CLUSTER_NAME="emr-eks-karpenter-kyuubi"
...
export DEFAULT_AZ=${AWS_DEFAULT_REGION}b
...
export S3_BUCKET="s3://emrekspdx/"
export EMR_NAMESPACE=emr
export KYUUBI_NAMESPACE=kyuubi
export KYUUBI_SA=emr-kyuubi
export EMRCLUSTER_NAME=${CLUSTER_NAME}-emr
export EMRCLUSTER_ID=$(aws emr-containers list-virtual-clusters | jq -r --arg name "$EMRCLUSTER_NAME" '.virtualClusters[] | select(.name == $name) | .id')
